...
sgtitle(['Increasing viewpoint invariance of face-selective units along the network hierarchies'])

% Viewpoint variation (Fig.S8a)
pos_subplot = [5:-1:1]; pos_img = [1:numIMG_view:41];
for ii = 1:5
    subplot(4,8,pos_subplot(ii)); imagesc(IMG_view(:,:,1,pos_img(ii))); axis image off; colormap(gray); caxis([0 255])
end
pos_subplot = [13:-1:9]; pos_img = [2:numIMG_view:42];
for ii = 1:5
...
% Face unit response (Conv5) (Fig.S8a)
...
ylim([0 mean(nanmean(act_Norm3D(cell_list_spe3,3,:),1),3)+std(nanmean(act_Norm3D(cell_list_spe3,3,:),1),[],3)./sqrt(numIMG_view)+0.2]); ylabel('Normalized Response (A.U.)'); title('Face unit response (Fig.S8a)')

% Average tuning curves of face units in each layer (Fig.S8f)
...
xlabel('Viewpoint difference (deg)'); ylabel('Normalized response (A.U.)'); legend([s1,s2,s3],'Conv3','Conv4','Conv5','Location','northeast'); title('Untrained network (Fig.S8f)');

% Viewpoint invariance (Fig.S8g)
...
xticklabels({'Conv3','Conv4','Conv5'}); xlabel('Network hierarchy'); ylabel('Invariance index'); title('Viewpoint invariance (Fig.S8g)')

% Number of invariant units (Fig.S8h)
...
xticklabels({'Conv3','Conv4','Conv5'}); xlabel('Network hierarchy'); ylabel('Ratio change of viewpoint-invariant units'); title('Number of units (Fig.S8h)')