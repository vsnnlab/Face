%% Spontaneous emergence of face-selectivity in untrained networks (Fig.1, Fig.S1-3)
...
sgtitle('Result 1 : DNN with randomly initialized weights (Fig 1b)')
...
sgtitle('Result 1 : Simiarity-controlled stimulus set (Fig 1c)')
% Figure 1c: Stimulus
...
array_lum = zeros(numCLS-1,numIMG);
for cc = 1:numCLS-1
    for ii = 1:numIMG
...
xticks([1:numCLS-1]); xticklabels(STR_LABEL); xlim([0.5 numCLS-1+0.5]); ylim([0 1]); ylabel('Luminance'); title('Mean luminance of image (Fig S1b)')

% Figure S1g: Intra-class image similarity 
disp(['Measure intra-class image similarity ... (~ 50 sec)'])
tic
array_intraSim = zeros(numCLS-1,numIMG*(numIMG-1)/2);
for cc = 1:numCLS-1
    iStep = 0;
    for ii = 1:numIMG
...
boxplot(array_intraSim'); ylim([-1 1]); xticklabels(STR_LABEL); xlim([0.5 numCLS-1+0.5]); ylabel('Image similarity'); title('Intra-class image similarity (Fig S1g)')

%% Face units in an untrained network (Fig 1d,e)
figure('units','normalized','outerposition',[0 0.5 1 0.5]);
sgtitle('Result 1 : Face units in an untrained network (Fig 1d,e)')
...
% In manuscript, FSI was measured using Tsao2006 set. If you want to reproduce the result of our paper, please request the image set to the corresponding author of Tsao 2006.
rep_shuf_mat_3D = reshape(rep_mat_3D(randperm(numel(rep_mat_3D))),size(rep_mat_3D));
fsi_mat = fun_FSI(rep_mat_3D);
fsi_shuf_mat = fun_FSI(rep_shuf_mat_3D);

subplot(2,6,[5,6,11,12]); hold on;
boxplot([fsi_mat,fsi_shuf_mat])
xticks([1:2]); xticklabels({'Untrained','Response shuffled'}); xlim([0.5 2.5]);
ylabel('Face-selectivity index (FSI)'); title('Single neuron tuning (Fig 1e, Fig S3)');

%% Face units distribution acorss filter (Fig S2b,d,e)
disp(['Find face unit in untrained networks when varying inital weight distribution ... (~ 1 min)'])
tmp_idx_FaceUnit = Cell_Idx{1,1,1,layerArray(5)};
[~,~,idx_FaceUnit_Filter] = ind2sub(array_sz(layerArray(5),:),tmp_idx_FaceUnit);
tmp_numFaceUnit_filter = zeros(array_sz(layerArray(5),3),1);
for ii = 1:array_sz(layerArray(5),3)
    tmp_numFaceUnit_filter(ii) = length(find(idx_FaceUnit_Filter == ii));
end

numFilter_existFaceUnit = length(find(tmp_numFaceUnit_filter>0));
[tmp_sort_num,tmp_order] = sort(tmp_numFaceUnit_filter,'descend');

% response matrix with face unit
figure('units','normalized','outerposition',[0 0 2/3 0.5]);
sgtitle('Result 1 : Response and distribution of face units (Fig S2b,d)')
load('Colorbar_Tsao.mat'); idx_subplot = [1 2 5 6]; 
for ii = 1:length(idx_subplot)
    subplot(2,4,idx_subplot(ii))
    act_reshape = reshape(squeeze(act_rand(:,:,tmp_order(ii),:)),[prod(array_sz(layerArray(5),1:2)) numCLS*numIMG]);
    [rep_mat,rep_mat_3D] = fun_ResZscoreRe(act_reshape,[1:prod(array_sz(layerArray(5),1:2))],numCLS,numIMG);
    rep_mat_3D_reshape = reshape(rep_mat_3D,[array_sz(layerArray(5),1), array_sz(layerArray(5),1), numCLS, numIMG]);
    
    MAT = mean(rep_mat_3D_reshape(:,:,1,:),4); MAT(isnan(MAT)) = 0;
    imagesc(MAT); axis image off; colormap(cmap); caxis([-1 1]); 
    title(['# ', num2str(tmp_order(ii))])
end; clearvars cmap

% XY location with face unit
tmpMatrix = zeros(array_sz(layerArray(5),1),array_sz(layerArray(5),2),numFilter_existFaceUnit);
for ii = 1:numFilter_existFaceUnit
    tmp_idx_FaceUnit_aFilter = tmp_idx_FaceUnit(idx_FaceUnit_Filter == tmp_order(ii));
    [I,J,~] = ind2sub([array_sz(layerArray(5),1),array_sz(layerArray(5),2),array_sz(layerArray(5),3)],tmp_idx_FaceUnit_aFilter);
    for jj = 1:length(tmp_idx_FaceUnit_aFilter)
        tmpMatrix(I(jj),J(jj),ii) = 1;
    end
end
load('Colorbar_Tsao.mat'); idx_subplot = [3 4 7 8];
for ii = 1:length(idx_subplot)
    subplot(2,4,idx_subplot(ii))
    imagesc(tmpMatrix(:,:,ii)); axis image off; colormap(cmap); caxis([0 1]); 
    title(['# ', num2str(tmp_order(ii))])
end; clearvars cmap

% Average distribution
figure('units','normalized','outerposition',[2/3 0 1/3 0.5]);
sgtitle('Result 1 : Distribution of face units (Fig S2d)'); title(['Averaged distribution'])
subplot(2,2,[1 2 3 4]); imagesc(mean(tmpMatrix./prod(array_sz(layerArray(5),1:2)),3)); axis image; colormap(hot); caxis([0 inf]); 
...
figure('units','normalized','outerposition',[0 0 1 0.5]);
sgtitle('Result 1 : Face units in untrained networks when varying inital weight distribution (Fig 1f-g)')
