%% Figure 2 : Preferred feature images of face-selective untis in untrained networks
load('PFI_XDream_ClsUnit.mat','PFI_XDream_mat')
load('PFI_RevCorr_ClsUnit.mat','PFI_RevCorr_mat')


%% Preferred feature images (PFI) (Fig 2b,2d)
Cell_PFI = cell(numCLS-1,2);
if Sim == 1                                                                % 1 : Fast version of PFI simulation. The saved PFI would be displayed.
    for cc = 1:numCLS-1
        for ii = 1:2
            switch ii
                case 1
...
elseif Sim == 2                                                            % 2 : Actual calculation process would be run. It takes around 10 minutes.
    ...
    Cell_PFI{1,1} = Cell_data{end};
    
    %% Reverse correlation (Fig 2d)
    % Stimulus parameters
...
sgtitle('Figure 2 : Preferred feature images (PFI) (Fig 2b,2d)')
for cc = 1:numCLS-1
    % stimulus
    ...
    imshow(uint8(Cell_PFI{cc,1})); axis image off;
...
ylabel('tSNE axis 2'); xlim([-50 50]); ylim([-50 50]); title('Raw pixel of input images (tSNE)');
...
ylim([-0.5 0.5]); ylabel(['Silhouette index']); title('Clustering index of input images');
...
% colormap(cmap(2:end,:)); cbh = colorbar; cbh.Ticks = 0.1:0.2:0.9 ; cbh.TickLabels = STR_LABEL(1:numCLS); legend off;
xlabel('tSNE axis 1'); ylabel('tSNE axis 2'); xlim([-50 50]); ylim([-50 50]); title('Response of Conv5 in untrained network (tSNE)');
...
xticks([1:numCLS-1]); xlim([0.5 numCLS-1+0.5]); xticklabels(STR_LABEL(orderSI));
ylim([-0.5 0.5]);ylabel(['Silhouette index']); title('Clustering index of Conv5 in untrained network');