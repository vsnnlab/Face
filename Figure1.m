...
%% Simiarity-controlled stimulus set (Fig 1b, S1b, S1g)
figure('units','normalized','outerposition',[0 0.5 1 0.5]);
sgtitle('Figure 1 : Simiarity-controlled stimulus set (Fig 1b)')
% Figure 1b: Stimulus
...
%% DNN with randomly initialized weights (Fig 1c)
net_rand = Cell_Net{1,1,1};
layers_ind = [2,10,14]; % target layers: conv1~5
vis_N = 3; % number of filters to visualize

figure('units','normalized','outerposition',[0 0 0.5 0.5]); colormap(gray)
sgtitle('Figure 1 : DNN with randomly initialized weights (Fig 1c)')
for ii = 1:length(layers_ind)
    ...
    for jj = 1:length(indtmpp)
        ...
        if jj == 2; title(['Pretrained Conv',num2str(2*ii-1),' filters'],'FontSize',fontSize_legend); end
        ...
        if jj == 2; title(['Untrained Conv',num2str(2*ii-1),' filters'],'FontSize',fontSize_legend); end
    end
end

%% Face units in an untrained network (Fig 1d,e,f)
figure('units','normalized','outerposition',[0 0.5 1 0.5]);
sgtitle('Figure 1 : Face units in an untrained network (Fig 1d-f)')
...
xticks([100:numIMG:1100]); xticklabels(STR_LABEL); c = colorbar; %c.Label.String = 'Response (z)';
ylabel('Unit indices'); title('Face-selective response (Fig 1d)')
...
ylabel('Response (z)'); title('Tuning curve of face-units (Fig 1e)'); legend(s,'Averaged','Location','northeast')
...
ylabel('Face-selectivity index (FSI)'); title('Single neuron tuning (Fig 1f)');

%% Face units in untrained networks when varying inital weight distribution (Fig 1g,h)
...
for nn = 1:NN
    for vv = 1:length(verArray)
        ...
        for ss = 1:length(stdArray)
            ...
            array_num_var(nn,vv,stdArray_order(ss)) = length(Idx_Face)./num_cell.*100;
...
sgtitle('Figure 1 : Face units in untrained networks when varying inital weight distribution (Fig 1g-h)')
...
ylabel('Ratio'); xlim([-0.2 0.2]); ylim([0 0.1])
...
xlabel('Weight'); ylabel('Ratio'); xlim([-0.2 0.2]); ylim([0 0.1])
...
title('Network initialization (Fig 1g)'); legend([s1,s2],'Gaussian','Uniform','Location','northeast')

% Figure 1h: variation of weights vs Number of face units
subplot(2,6,[5,6,11,12]); hold on
if NN == 1
...
ylim([0 1.5]); ylabel('Ratio of face units (%)'); xlabel('Variation of weights'); 
title('Emergence of face units (Fig 1g)'); legend([s1,s2],'Gaussian','Uniform','Location','northeast')