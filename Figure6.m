...
NN_trained = 5; 
TT = 4; 
indLayer = length(layerArray);
reN = 10;       
num_minTotal = 100;

Cell_Idx_trained = cell(TT,NN);
Cell_FSI_trained = cell(TT,NN);
Cell_SVM_trained = cell(TT,NN);
Cell_ER_trained = cell(TT,NN,3);
Cell_var_axis = cell(3,1);

%% Preferred feature images (PFI) (Fig 6b-f)
for nn = 1:NN_trained
    tic
...
array_num = zeros(NN_trained,TT);
array_fsi = zeros(NN_trained,TT); 
array_svm = zeros(NN_trained,TT); 
array_er = zeros(NN_trained,TT,3); 
...
% title('Face detection performance (Fig 6d)')
...
subplot(4,6,[17,18,23,24]); hold on; vtype = 2;
tmpScale = Cell_var_axis{vtype}(end)-Cell_var_axis{vtype}(end-1);
bar([1],mean(tmpScale.*array_er(:,1,vtype),1),0.5,'facecolor',[0 0 0])
errorbar([1],mean(tmpScale.*array_er(:,1,vtype),1),std(tmpScale.*array_er(:,1,vtype),1),'k')
bar([2],mean(tmpScale.*array_er(:,4,vtype),1),0.5,'facecolor',[0 0 1])
